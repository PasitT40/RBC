...
### Client-side WebP Conversion (ก่อน upload Firebase Storage)

รูปทุกประเภท (ยกเว้นโลโก้แบรนด์ที่ต้องการ transparency) จะถูก convert เป็น WebP อัตโนมัติใน browser ก่อน upload — ลดขนาดไฟล์ได้ 30–50% เทียบกับ JPG/PNG

**Flow:**
```
user เลือกไฟล์ → validate dimension/size/format → canvas.toBlob('image/webp', quality)
→ ตรวจขนาดหลัง convert ≤ maxSizeKB → upload to Firebase Storage
```

**กรณีพิเศษ — โลโก้แบรนด์ (PNG เท่านั้น):**
PNG ที่มี transparency แปลง WebP ได้ (`image/webp` รองรับ alpha channel) แต่เพื่อความปลอดภัยกับ browser เก่า ให้เก็บเป็น PNG ต่อไปสำหรับ logo

**Composable: `useImageUpload.ts`** (สร้างใหม่ใน `app/composables/`)

```ts
async function processImage(file: File, constraint: ImageConstraint): Promise<Blob> {
  // 1. วาด file ลง canvas ด้วย constraint.width × constraint.height (center-crop ถ้า ratio ไม่ตรง)
  // 2. แปลงเป็น WebP ด้วย canvas.toBlob('image/webp', 0.85)
  // 3. ตรวจขนาด Blob ≤ constraint.maxSizeKB * 1024 (ถ้าเกิน ลด quality จนผ่าน)
  // 4. return Blob พร้อม upload
}
```

**ไฟล์ที่ Firebase Storage จะได้รับ:**

| ประเภท | ก่อน | หลัง |
|--------|------|------|
| Banner | JPG/PNG ≤500 KB | WebP ≤500 KB (~200–300 KB จริง) |
| หมวดหมู่ | JPG/PNG ≤300 KB | WebP ≤300 KB |
| โลโก้แบรนด์ | PNG ≤300 KB | PNG ≤300 KB (ไม่แปลง) |
| ภาพสินค้า | JPG/PNG ≤300 KB | WebP ≤300 KB |
| รีวิว | JPG/PNG ≤300 KB | WebP ≤300 KB |
...
interface ImageConstraint {
  ...
  maxSizeKB: number    // max file size หลัง convert
  keepPng?: boolean    // true = ไม่แปลง WebP (ใช้กับ brand logo)
...
ใช้งาน:
```vue
<!-- Banner upload — แปลง WebP -->
<form-vee-file-input
  name="bannerImage"
  :constraint="{ width: 1920, height: 600, maxSizeKB: 500, label: '1920 × 600 px · ≤500 KB' }"
/>
...
| ไฟล์ | งาน |
|------|-----|
| `app/composables/useImageUpload.ts` | สร้างใหม่ — WebP conversion + validation logic |
