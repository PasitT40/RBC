...
async function confirmSale(productId, soldPrice, soldChannel, fee = 0, idempotencyKey) {
  ...
  await db.runTransaction(async (tx) => {
    ...
    tx.set(ledgerRef, buildLedgerPayload("SALE_APPLIED", orderRef.id, productId), { merge: true });
  });
  return orderRef.id;
}
...
async function main() {
  ...
  const saleLedger = saleLedgerSnap.data();
  ...
  const revertLedger = revertLedgerSnap.data();
  if (revertLedger.entity_type !== "order" || revertLedger.entity_id !== orderId || revertLedger.operation_key !== `SALE_REVERTED_${orderId}`) {
    throw new Error("undoSale: revert ledger payload invalid");
  }
